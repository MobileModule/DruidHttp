...
public class RequestQueue {
    ...
    private final BlockingQueue<DruidHttpRequest> mUnFinishQueue = new LinkedBlockingDeque<>();
    private final BlockingQueue<DruidHttpRequest> mRequestQueue = new PriorityBlockingQueue<>();
    private final Map<DruidHttpRequest, Messenger> mMessengerMap = new LinkedHashMap<>();
     ...
     *                 {@link DruidHttpRequest} results in an A with what, please distinguish which is the result of the
     *                 {@link DruidHttpRequest}.
     * @param request  {@link DruidHttpRequest}.
     * @param listener {@link OnResponseListener}.
     * @param <T>      {@link T}.
     */
    public <T> void add(int what, DruidHttpRequest request, OnResponseListener listener) {
    ...
    public void cancelBySign(Object sign) {
        synchronized (mUnFinishQueue) {
            for (DruidHttpRequest request : mUnFinishQueue) request.cancelBySign(sign);
        }
    }
    ...
    public void cancelAll() {
        synchronized (mUnFinishQueue) {
            for (DruidHttpRequest request : mUnFinishQueue) {
                request.cancel();
            }
        }
    }
